...
static unsigned char prevBumperValue;
 ...
 * @brief Checks for a bumper event -- was one of the bumpers pressed?
 ...
 *          - return TRUE
 *      - return FALSE
 * @version 1 -- no debouncing included
 * @param
...
uint8_t checkBumpers(void)
{
    ...
    unsigned char curBumperValue = Roach_ReadBumpers();
    ...
    if(BumperChange)            // if any bumper is pressed, returns an event
    {
        ...
        ThisEvent.EventType = BUMPER_CHANGE;
...
{
    unsigned int LightValue = Roach_LightLevel();
    LIGHT_STATE LightState = GRAY;
    ES_Event ThisEvent;
    ...
    if(LightState != GRAY)
    {
        ...
        ThisEvent.EventParam = LightState;     // 1 = DARK, 2 = LIGHT
        PostLightService(ThisEvent);
        return TRUE;
    }
    return FALSE;
}
