 ...
 * File:   AD.c
 * Author: mdunne
 * @note Edited by Nick Kuipers for B.O.A.T project
 *      - Begins on line 398 to circumvent the low voltage lockout. Rather than sticking the system
 *      - into an eternal while loop and deactivating everything, the "uh-oh" LED will be triggered and remain asserted
 *      - until system reset
 *
...
#include <BOAT_PinDefs.h>   // added by Nick Kuipers for Low voltage LED
...
void __ISR(_ADC_VECTOR) ADCIntHandler(void)
{
    ...
    if (SampleCount > PointsPerBatSamples) {//if sample time has passed
        ...
        if ((CurFilt_BatVoltage <= BAT_VOLTAGE_LOCKOUT) && (PrevFilt_BatVoltage <= BAT_VOLTAGE_LOCKOUT) && (AD_ReadADPin(BAT_VOLTAGE_MONITOR) > BAT_VOLTAGE_NO_BAT)) {
            // Change from provided AD.c : if the battery voltage drops below the lockout threshold a specific pin will get toggled rather than
            // the entire system shutting down (in essence: turn on the "uh-oh" LED)
            LOW_VOLTAGE_LED_TRIS = 0;
            LOW_VOLTAGE_LED = 1;
