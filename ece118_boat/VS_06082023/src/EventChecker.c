...
{
    ES_Event ThisEvent;
    static unsigned char last2kBeaconValue = 0;               // can be 0 or 1 (LOW or HIGH)
     ...
     *      above the high threshold. This indicates a move from low to high
     */
    if(last2kBeaconValue == 0 && currentBeaconValue > BEACON_2K_HIGH_THRESHOLD)
    {
        last2kBeaconValue = 1;
        ThisEvent.EventType = BEACON_EVENT_2K;
        ThisEvent.EventParam = last2kBeaconValue;
        PostBeaconService(ThisEvent);
        return TRUE;
    }
    
    if(last2kBeaconValue == 1 && currentBeaconValue < BEACON_2K_LOW_THRESHOLD)
    {
        last2kBeaconValue = 0;
        ThisEvent.EventType = BEACON_EVENT_2K;
        ThisEvent.EventParam = last2kBeaconValue;
