...
/**
 * @brief Composes and sends a full packet to the distant microcontroller
...
{
    // converts void ptr to unsigned char ptr. Reading each character requires ptr dereference
    unsigned char* temp = (unsigned char*)inputString;

    // 1st Byte: Head = 0xCC
    if(*temp++ != HEAD)
        return 1;

    // 2nd Byte: LENGTH = variable
    if(*temp < 1 || *temp > MAXPAYLOADLENGTH)
        return 1;
    rxPacket.len = *temp++;

    // 3rd Byte: MessageID = variable (see enum in MessageIDs.h)
    if(*temp = 0)
        return 1;    // ID_ERROR
    payloadPos_RX = 0;
    rxPacket.messageID = *temp;
    rxPacket.checksum = Protocol_CalcIterativeChecksum(receivedByte, 0);  // begin checksum calcs
    rxPacket.payload[payloadPos_RX++] = *temp++;      // first byte of payload = messageID

    // 4th Byte(s): Payload = variable data and # of bytes (depending on len)
    while(*temp != TAIL)
    {
        if(payloadPos_RX == rxPacket.len)
            return 1:    // tail not in expected position
        rxPacket.checksum = Protocol_CalcIterativeChecksum(*temp, rxPacket.checksum);
        rxPacket.payload[payloadPos_RX++] = *temp++;
    }

    // Checksum byte
    if(*temp++ != rxPacket.checksum)
        return 1;           // Error -- incorrect checksum
    RXPacketComplete = 1;
...
/**
 * @brief Determines operation to perform based on the messageID of the received packet from
 *      the PIC
...
int Protocol_Operate(Packet *op_packet)
{
    ...
    {
        case ID_STANDBY:          // initial message received from UNO
        {
            //setLEDValues(op_packet->payload[1]);         // by default an LEDS_SET message has 1 byte of payload
            // @TODO run through initialization, verify connection with all sensors
            // if(ARD_Init() == ERROR)
            //      txPacket.payload[0] = ID_ERROR;
            //      txPacket.payload[1] = 4;
            //      txPacket.payload[2] = 0;
            //      txPacket.payload[3] = 4;
            //      Protocol_SendPacket(0x03, ID_ERROR, &txPacket.payload)

            /*
            *   The following code is directly inspired from my previous ID_PONG case from
            *       ECE 121.
            *   The PIC sends a hard-coded "ping" message (named ID_STANDBY) with 4 characters. If the
            *       initialization is successful AND the communications interface is good, the checksum
            *       will be appropriately calculated here and match the hard-coded reference held by
            *       the PIC, indicating the PIC is good to move to the STANDBY state.
            */
            txPacket.payload[0] = ID_STANDBY_AYE;
            unsigned int pong = 0;
            unsigned int temp = 0;
            ...
            Protocol_SendPacket(0x05, ID_STANDBY_AYE, &txPacket.payload);
            break;
        }
        case ID_ENGAGE:
        {
            // @TODO Begin ranging protocol
            // ARD_RangingLoop();      , OR , RangingFlag = true;   -> main loop waits for flag to be set to true
            break;
        }
        default:
        {
