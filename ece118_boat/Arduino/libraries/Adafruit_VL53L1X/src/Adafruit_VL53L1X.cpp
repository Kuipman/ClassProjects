...
bool Adafruit_VL53L1X::begin(uint8_t i2c_addr, TwoWire *theWire, bool debug) {
    //Serial.println("begin() entry");
  if (_shutdown_pin != -1) {
      //Serial.println("Shutdown pin entry");
  ...
  theWire->begin();
  //Serial.println("The wire began");

  MyDevice.I2cHandle = theWire;
  //Serial.println("I2CHandle passed");
  dev_i2c = theWire;
  //Serial.println("dev_i2c passed");

  vl_status = InitSensor(i2c_addr * 2);    // Behold, the problem child
  //Serial.println("vl_status passed");
  if (vl_status != VL53L1X_ERROR_NONE) {
      //Serial.println("VL53L1X_ERROR_NONE");
    return false;
  }

  if (sensorID() != 0xEACC) {
      //Serial.println("SensorID = 0xEACC");
    return false;
  }

  // Running test function - NK
  ...
  */
  //Serial.println("Returning true. Leaving begin()");
  return true;
}
