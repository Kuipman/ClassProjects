...
scale_factor_accelerometer= 9.8./max(A)
sF_magnetometer =47835 ./ max(M)

nAx=scale_factor_accelerometer(1)*(A(:,1)- mean(A(:,1)))
nAy=scale_factor_accelerometer(2)*(A(:,2)-mean(A(:,2)))
nAz=scale_factor_accelerometer(3)*(A(:,3)-mean(A(:,3)))

calibratedA=[nAx nAy nAz]
norm_CalibratedA= sqrt((calibratedA(:,1).^2)+(calibratedA(:,2).^2)+(calibratedA(:,3).^2))
norm_A= sqrt((A(:,1)*scale_factor_accelerometer(1)).^2+(A(:,2)*scale_factor_accelerometer(2)).^2+(A(:,3)*scale_factor_accelerometer(3)).^2)
