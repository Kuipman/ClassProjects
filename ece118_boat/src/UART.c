...
// temporary variable to pull bad chars from RX FIFO
char tempChar;
...
void initializeUART(unsigned long baudrate)
{
    ...
    U1STAbits.UTXISEL = 0;    //triggers when TX FIFO has at least one empty space 
 ...
 * @return ASCII character as an integer, or a 0 if RX buffer is empty
 */
int _mon_getc(int canblock)
{
    if(!getBufferSize(&RXBUF) && !RXBUF.isFull)
        return 0;             // RX buffer empty, returns null character
...
unsigned char GetChar(CircBuffer* cB)
{
    ...
    char tempChar = cB->buffer[cB->head];
...
void IRQ_Handler_UART1_RX()
{
    ...
    {
        tempChar = U1RXREG & 0xFF;               // Sends bad data to a placeholder to clear up the register
        U1STAbits.FERR = 0;
        U1STAbits.PERR = 0;
    }
