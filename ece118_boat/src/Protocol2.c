...
uint8_t BuildRxPacket (unsigned char receivedByte, Packet *rxPacket, unsigned char reset)
{
    ...
    switch (PSTATE)
    {
        ...
        case PSM_PAYLOAD:
             ...
             * */
            PSTATE = PSM_START;
            return 2;      // end of packet protocol
            break;
            /*
        case PSM_CHECKSUM:
...
/**
 * Tests the UART interface between the PIC and distant microcontroller
 *  - Initializes the board, boat, and communications interface
 *  - Leverages the existing serial library to call putchar constantly
 *      and send individual characters over UART. If the UART TX works correctly,
 *      not only will the PC console receive the characters but the Arduino will as well.
 *      The Arduino's test bench will output received characters (once a newline is received)
 *      to its respective console.
 */
