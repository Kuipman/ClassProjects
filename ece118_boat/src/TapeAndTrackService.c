...
uint8_t InitTapeAndTrackService(uint8_t Priority)
{
    ...
    lastTrackValue = 0;
    timerRunning = FALSE;
    StateEvent.EventType = ES_NO_EVENT;
...
//        }
        case TAPE_EVENT:
        {
            StateEvent.EventParam = ThisEvent.EventParam;
            if(timerRunning == FALSE)
            {
                timerRunning = TRUE;
                ES_Timer_InitTimer(4, 50);   // Start the 50ms debounce timer
            }
            break;
        }
        case ES_TIMEOUT:
        {
            timerRunning = FALSE;
            if(lastTapeValue != StateEvent.EventParam)
            {
                printf("tapes not the same last; %i, state;%i", lastTapeValue, StateEvent.EventParam);
                lastTapeValue = StateEvent.EventParam;
                StateEvent.EventType = TAPE_EVENT;
                PostHSM(StateEvent);
            }
            else
            {
                printf("tapes the same last; %i, state;%i", lastTapeValue, StateEvent.EventParam);
            }
            break;
        }
        default:
        ...
        //{
            //PostHSM(ThisEvent);    // this is getting called relentlessly
            //break;
        //}
    }
    return ReturnEvent;
