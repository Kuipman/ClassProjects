...
typedef enum {
    ...
    DEVICE_RESET,        // Generated by Switch ON -> OFF
    BUMPER_EVENT,        // Generic Bumper Event
    BUMPER_PRESS,        // Generated when no bumpers are pressed, then at least one is
    BUMPER_RELEASE,      // Generated when at least one bumper is pressed, then all are released
    BUMPER_CHANGE,       // Generated when at least one bumper is pressed, then a change occurs but at least one bumper remains pressed
    TAPE_EVENT,          // Generic Tape Event, generated when a tape sensor changes state. Parameter is current value of tape sensors
    TRACKWIRE_EVENT,     // Generated when Trackwire state changes (0 -> 1 or 1 -> 0)
    BEACON_EVENT_2K,
    TEST_EVENT,
    //BEACON_EVENT_2K,     // Generated when 2K beacon state changes (0 -> 1 or 1 -> 0)
    BEACON_EVENT_1_5K,   // Generated when 1.5 beacon state changes (0 -> 1 or 1 -> 0)
    // SSM States
    SSM_FINISHED,        // Event generated when a Sub-State Machine has reached its final state
...
#define EVENT_CHECK_LIST checkBumpers, checkTapeSensors, check2kBeacon, check1_5kBeacon, updateTOF, checkSwitch     //TemplateCheckBattery
...
#define TIMER0_RESP_FUNC PostBumperService      // Debounce timer for bumpers
#define TIMER1_RESP_FUNC PostHSM                // Generic timer for use in HSM/SSMs
#define TIMER2_RESP_FUNC PostHSM                // Generic timer for use in HSM/SSMs
#define TIMER3_RESP_FUNC PostHSM                // Generic timer for use in HSM/SSMs
#define TIMER4_RESP_FUNC PostTapeAndTrackService    // Debounce for Tape Sensors (sensitive calibration)
