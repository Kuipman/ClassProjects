...
int main()
{
    ...
    //Test 7: CircBuffer User Input Test
    printf("\n===============\n");
    printf("Test 7: User Input Portion\n");
    printf("Enter # to exit. Enter a character below.\n");
    CircBuffer RXBUF, TXBUF;    //for user input section
    initializeCircBuff(&RXBUF);
    initializeCircBuff(&TXBUF);
    testChar = 0;
    while (testChar != '#')
    {
        testChar = getchar();
        PutChar(&RXBUF, testChar);
        PutChar(&TXBUF, GetChar(&RXBUF));
        printf("%c", GetChar(&TXBUF));
    }
