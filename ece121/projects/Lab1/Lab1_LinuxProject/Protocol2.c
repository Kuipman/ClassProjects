...
int Protocol_SendDebugMessage(char *Message)
{
    ...
    int len = 0; 
    while(*temp != '\n')         // As tx payload fills, increment a counter to determine length
    {
        unsigned char* temp = (unsigned char*)Message + len;
        txPacket.payload[len] = *temp;
        len++;
    }
    Protocol_SendPacket(len, ID_DEBUG, &txPacket.payload);
}
...
int main()
{
    ...
    //char* initialMessage = constructInitialMessageString();
    ...
    while(1)
    {
        ...
        if(currentBufferSize('R'))    //RX circular buffer in UART not empty
        {
