...
/**
 * @brief Uses Timer5 to block code until a certain amount of microseconds have
 *      passed, determined by input parameter msCount.
 *      This is performed by:
 *          - setting a value to the current microsecond time
 *          - running a continuous while loop until the elapsed time since
 *              that measurement is 10 microseconds
 * @msCount, number of microseconds to block code by 
 * @return nothing
 */
void FreeRunningTimer_DelayMicroseconds(int usCount)
{
    unsigned int startTime = FreeRunningTimer_GetMicroSeconds();
    while((FreeRunningTimer_GetMicroSeconds() - startTime) < usCount) {}
}
