...
int Message_Encode(char *message_string, Message message_to_encode)
{
    ...
    char checkString[MESSAGE_MAX_LEN] = {};
    int checkSum;
    switch(m_type)
    {
        case MESSAGE_CHA:
            sprintf(checkString, "%s,%i", "CHA", message_to_encode.param0);
            checkSum = Message_CalculateChecksum(checkString);
            sprintf(message_string, "$%s,%u*%02X\n", "CHA", message_to_encode.param0, checkSum);
            break;
        case MESSAGE_ACC:
            sprintf(checkString, "%s,%i", "ACC", message_to_encode.param0);
            checkSum = Message_CalculateChecksum(checkString);
            sprintf(message_string, "$%s,%u*%02X\n", "ACC", message_to_encode.param0, checkSum);
            break;
        case MESSAGE_REV:
            sprintf(checkString, "%s,%i", "REV", message_to_encode.param0);
            checkSum = Message_CalculateChecksum(checkString);
            sprintf(message_string, "$%s,%u*%02X\n", "REV", message_to_encode.param0, checkSum);
            break;
        case MESSAGE_SHO:
            sprintf(checkString, "%s,%i,%i", "SHO", message_to_encode.param0, message_to_encode.param1);
            checkSum = Message_CalculateChecksum(checkString);
            sprintf(message_string, "$%s,%u,%u*%02X\n", "SHO", message_to_encode.param0, message_to_encode.param1, checkSum);
            break;
        case MESSAGE_RES:
            sprintf(checkString, "%s,%i,%i,%i", "RES", message_to_encode.param0, message_to_encode.param1, message_to_encode.param2);
            checkSum = Message_CalculateChecksum(checkString);
            sprintf(message_string, "$%s,%u,%u,%u*%02X\n", "RES", message_to_encode.param0, message_to_encode.param1, message_to_encode.param2, checkSum);
            break;
        case MESSAGE_NONE:
            return 0;
        case MESSAGE_ERROR:
...
void resetValues()
{
    for(int i = 0; i < array_pointer+10; i++)
        decoded_message[i] = '\0';
    array_pointer = 0;
