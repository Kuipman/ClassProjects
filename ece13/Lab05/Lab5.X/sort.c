...
int main(void)
{
    ...
    printf("\n\nWelcome to NKuipers' sort.c, compiled on %s %s.\n", __DATE__, __TIME__);
...
ListItem *SelectionSort(ListItem* list)
{
    ...
    char *temp;      //utilized whenever FU and S need to be swapped
    while(FU != NULL)
    {
        while(S != NULL)
        {
            //if S pointer is smaller than the FU pointer, switch the data values
...
ListItem *InsertionSort(ListItem* list)
{
    ListItem *FU = LinkedListGetLast(list);
    ListItem *S = FU;
    ListItem *LU = FU->previousItem;
    while(LU != NULL)
    {
        //First, checks if the element to the left of FU is smaller (comes first alphabetically)
        if(strcmp(FU->data, LU->data) > 0)
            {
                FU = LU;
                S = FU;
                LU = LU->previousItem;
        ...
        //If element to the left is larger (comes after FU) then we utilize the S pointer
        while(S != NULL)
        {
            if(S->nextItem == NULL || strcmp(S->nextItem->data, LU->data) > 0)
            {
